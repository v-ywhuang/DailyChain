...
export default function DashboardClient({ user }: DashboardClientProps) {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden bg-gradient-to-br from-slate-900 via-indigo-900 to-slate-900">
      {/* Aurora Background - GPU 加速优化 */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none gpu">
        <motion.div
          className="absolute top-1/4 left-1/3 w-96 h-96 bg-blue-500/20 rounded-full blur-3xl gpu"
          animate={{ scale: [1, 1.1, 1], x: [-10, 10, -10] }}
          transition={{ duration: 20, repeat: Infinity, ease: 'easeInOut' }}
          style={{ willChange: 'transform' }}
        />
        <motion.div
          className="absolute bottom-1/4 right-1/3 w-96 h-96 bg-purple-500/20 rounded-full blur-3xl gpu"
      ...
      <div className="relative z-10">
        ...
        <motion.nav
          ...
          className="sticky top-0 z-50 backdrop-blur-2xl bg-gradient-to-r from-slate-900/80 via-purple-900/60 to-slate-900/80 border-b border-white/10 shadow-2xl shadow-purple-500/10 gpu-accelerated"
        >
          ...
          >
            {[
              ...
              },
              {
                icon: '✅',
                label: '今日完成',
                value: `${habits.filter((h) => h.checked_today).length}/${
                  habits.length
                }`,
              ...
              },
              {
                icon: '📊',
                label: '总打卡数',
                value: `${stats?.total_check_ins || 0}次`,
                color: 'from-blue-500 to-cyan-500',
              },
              {
                icon: '🎯',
                label: '习惯数量',
                value: `${stats?.active_habits || 0}个`,
              ...
              >
                <div
                  className="absolute inset-0 bg-gradient-to-r opacity-0 group-hover:opacity-20 rounded-3xl blur-xl transition-opacity"
                  style={{
                    backgroundImage: `linear-gradient(to right, var(--tw-gradient-stops))`,
                  }}
                />
                <div className="relative backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl sm:rounded-3xl p-4 sm:p-6 gpu-accelerated">
            ...
            <div className="space-y-4">
              {habits.map((habit, index) => (
                <motion.div
                  ...
                  className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl sm:rounded-3xl p-4 sm:p-6 cursor-pointer hover:bg-white/15 transition-all gpu-accelerated"
                >
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between text-xs sm:text-sm text-white/70 mb-2">
                      <span>进度</span>
                      <span>
                        {habit.total_check_ins} / {habit.target_days || 0} 天
                      </span>
                    </div>
